...
for iteration in xrange(ITERS):
    ...
    for iter_d in xrange(CRITIC_ITERS):
        ...
        netD.zero_grad()
        sigma = max(0.1 * (1 - float(iteration) / ITERS), 0.01)  # 迭代越多噪声越小

        # train with real + 加噪声
        real_noisy = real_data_v + torch.randn(real_data_v.size()).cuda(gpu) * sigma if use_cuda else torch.randn(real_data_v.size()) * sigma
        D_real = netD(real_noisy)
        D_real = D_real.mean()
        ...
        fake_noisy = inputv + torch.randn(inputv.size()).cuda(gpu) * sigma if use_cuda else torch.randn(inputv.size()) * sigma
        D_fake = netD(fake_noisy)
        D_fake = D_fake.mean()
        D_fake.backward(one)


        # PYJ判别器总损失（无 gradient penalty）
